#!/usr/bin/env python3
"""
Generate deterministic, accessible SVG charts for blog posts.

Source: claude-blog (MIT) — AgriciDaniel/claude-blog,
`skills/blog-chart/scripts/generate_chart_svg.py`. Copiado e adaptado para este
blog: o texto do gráfico cai em `currentColor` (sem media query de tema), e o
CSS vive em `src/css/custom.css` (`.blog-chart`). Resto: MIT, sem garantia.

Usage:
    python3 generate_chart_svg.py --input chart.json --output chart.html --json
    python3 generate_chart_svg.py --input chart.json --format mdx
...
def render_figure(config: dict, output_format: str = "html") -> str:
    ...
    figcaption += "."
    # Local (lucabelezal): sem --chart-muted inline nem media query prefers-color-scheme.
    # Texto cai em currentColor e herda o tema (light/dark) definido em custom.css.
    svg = f'''<figure class="blog-chart">
<svg viewBox="0 0 {WIDTH} {HEIGHT}" style="max-width: 100%; height: auto; font-family: 'Inter', system-ui, sans-serif;" role="img" aria-labelledby="{esc(title_id)} {esc(desc_id)}">
  <style>
